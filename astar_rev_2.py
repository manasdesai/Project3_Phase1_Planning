...
###############################################################################
def astar(occupancy_grid, color_occupancy_grid, start, goal, scale_factor, safety_margin, step_size, spatial_resolution, angular_resolution, h=euclidean_ground_distance, logging=False): 
...
def animate_search(color_occ_grid, search_array_history, path):
    ...
    ax.imshow(color_occ_grid.T, origin="lower", cmap="inferno", alpha=1.0,
    ...
    search_space = ax.imshow(np.array(first_frame_list_of_lists).T, origin="lower", cmap=search_cmap,
                               norm=search_norm, alpha=0.6,
                               extent=[0, color_occ_grid.shape[0], 0, color_occ_grid.shape[1]])
    ...
    # START marker
    start_marker = Line2D([0], [0], marker='o', color='w', markerfacecolor='chartreuse', markersize=8, label='Start')
    # GOAL marker
    goal_marker = Line2D([0], [0], marker='o', color='w', markerfacecolor='magenta', markersize=8, label='Goal')
    # PATH marker
    path_marker = Line2D([0], [0], color='red', lw=2, label='Final Path')
    # PLACE the legend in the top right corner
    ax.legend(handles=[closed_patch, open_patch, start_marker, goal_marker, path_marker], loc='upper right')
    ...
    anim = FuncAnimation(fig, update_frame, frames=len(search_array_history),
                         interval=200, blit=False, repeat=True)

    # Save the animation as a GIF.
    anim.save("astar_animation_2.gif", writer="pillow", fps=5)
    print("Animation saved as astar_animation_2.gif")

    # Save the animation as an MP4.
    anim.save("astar_animation_2.mp4", writer="ffmpeg", fps=5)
    print("Animation saved as astar_animation_2.mp4")
...
if __name__ == "__main__":
    ...
    color_occupancy_grid = generate_occupancy_grid_for_plotting(NEW_WORKSPACE_DIMENSION, SPATIAL_RESOLUTION, SCALE_FACTOR, safety_margin)
    ...
    if path:
        ...
        print(f"The final path in units of (mm, mm, deg) is {[coordinate_transformation_inverse(p, SPATIAL_RESOLUTION, ANGULAR_RESOLUTION) for p in path]}")
